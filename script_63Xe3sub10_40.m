...
FNAME='63X objective, higher noise-background/max_150112 ecadgfp sqhmcherry_e3_xImaging SD GFP sub10-40.tif';
...
for i=1:n
    ...
    if check<2 %if we are not completely skipping the marker checks
        [check imgshift]=checkMarkers(BW,1,imgadapt(:,:,i));gcf;title(sprintf('File = %s, frame %d / %d',FNAME,i,n));
        if(check==1) %check marker objects for fidelity
            BWL=BW;
            [BW,imgshift]=modifyMarkers(BWL,1,imgadapt(:,:,i),i); %modify marker objects for fidelity as necessary
        end
    else
